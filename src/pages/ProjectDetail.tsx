...
const ProjectDetail = () => {
  ...
  if (!project) {
    return <main className="min-h-screen flex items-center justify-center">
        <SEO title="Project Not Found" description="The requested project could not be found. Browse our premium residential plots and properties." url={`https://www.arsdevelopers.co.in/project/${id}`} />
  ...
  return <main className="min-h-screen">
      <SEO title={`${project.name} - Premium Plots in ${project.location.split(',')[0]}`} description={`${project.description} Plot sizes: ${project.plotSizes.join(', ')}. Starting from ${project.startingPrice}. Contact ARS Developers at +91-7065389036`} keywords={`${project.name}, plots in ${project.location.split(',')[0]}, ${project.amenities.slice(0, 5).join(', ')}, ARS Developers, residential plots`} url={`https://www.arsdevelopers.co.in/project/${project.id}`} image={project.mainImage} type="product" structuredData={projectStructuredData} />
      <Navbar />
      ...
      <section className="relative min-h-[85vh] overflow-hidden pt-20">
        ...
        <div className="absolute inset-0">
          ...
          <motion.img src={project.mainImage} alt={`${project.name} - Premium residential plots by ARS Developers`} className="w-full h-full object-cover" initial={{
          scale: 1.1
        }} animate={{
          scale: 1
        }} transition={{
        ...
        <div className="relative z-20 min-h-[85vh] flex flex-col justify-center items-center text-white px-4 py-12">
          ...
          duration: 0.8
            ...
            duration: 0.5
              ...
              <Button variant="ghost" className="mb-8 text-white/90 hover:text-accent hover:bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-6" onClick={() => navigate("/")}>
            ...
            {/* Project badge */}
            {project.highlights && project.highlights.length > 0 && <motion.div initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
      ...
      {project.highlights && project.highlights.length > 1 && <section className="py-16 bg-gradient-to-b from-primary/5 to-background">
          ...
          once: true
              ...
              {project.highlights.slice(1).map((highlight, index) => <motion.div key={index} initial={{
            opacity: 0,
            y: 20
          }} whileInView={{
            opacity: 1,
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Video Section */}
          {project.video && <motion.div initial={{
          opacity: 0,
          y: 30
        }} whileInView={{
          opacity: 1,
          ...
          once: true
              ...
              <div className="rounded-3xl overflow-hidden shadow-luxury">
                <video controls className="w-full" poster={project.mainImage} aria-label={`${project.name} project walkthrough video`}>
          ...
          once: true
            ...
            <div className="grid md:grid-cols-2 gap-10 items-start">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {project.amenities.map((amenity, index) => <motion.div key={index} initial={{
                opacity: 0,
                x: -20
              }} whileInView={{
                opacity: 1,
              ...
              {project.amenitiesImage && <motion.div initial={{
              opacity: 0,
              x: 20
            }} whileInView={{
              opacity: 1,
              ...
              once: true
                  ...
                  <img src={project.amenitiesImage} alt={`${project.name} amenities - ${project.amenities.slice(0, 3).join(', ')}`} className="w-full h-full object-cover" />
                </motion.div>}
            </div>
          </motion.div>

          {/* Layout Plans */}
          {(project.keyPlanImage || project.plotLayoutImage) && <motion.div initial={{
          opacity: 0,
          y: 30
        }} whileInView={{
          opacity: 1,
          ...
          once: true
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {project.keyPlanImage && <motion.div initial={{
              opacity: 0,
              scale: 0.95
            }} whileInView={{
              opacity: 1,
              ...
              once: true
                    ...
                    <div className="p-4 bg-white">
                      <img src={project.keyPlanImage} alt={`${project.name} key plan - location map`} className="w-full rounded-xl" />
                    </div>
                  </motion.div>}
                
                {project.plotLayoutImage && <motion.div initial={{
              opacity: 0,
              scale: 0.95
            }} whileInView={{
              opacity: 1,
              ...
              once: true
                    ...
                    <div className="p-4 bg-white">
                      <img src={project.plotLayoutImage} alt={`${project.name} plot layout - site plan`} className="w-full rounded-xl" />
                    </div>
                  </motion.div>}
              </div>
            </motion.div>}
          ...
          {/* Booking Plan */}
          {project.bookingPlanImage && <motion.div initial={{
          opacity: 0,
          y: 30
        }} whileInView={{
          opacity: 1,
          ...
          once: true
              ...
              <div className="bg-card rounded-2xl overflow-hidden shadow-luxury max-w-2xl">
                <img src={project.bookingPlanImage} alt={`${project.name} booking and payment plan`} className="w-full" />
              </div>
            </motion.div>}
          ...
          once: true
            ...
            <div className="rounded-2xl overflow-hidden shadow-luxury">
              <iframe src={project.mapEmbedUrl} width="100%" height="450" style={{
              border: 0
            }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`${project.name} location map`} />
            </div>
            
            <div className="mt-6 text-center">
              <Button size="lg" className="bg-accent hover:bg-accent/90 text-white rounded-full" onClick={() => window.open(project.mapUrl, "_blank")}>
          ...
          once: true
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <Button size="lg" className="bg-accent hover:bg-accent/90 text-primary font-bold rounded-full px-8 py-6 text-lg shadow-lg shadow-accent/30" onClick={() => window.location.href = "tel:+917065389036"}>
                  <Phone className="w-5 h-5 mr-2" />
                  Call Now
                </Button>
                
                <Button size="lg" variant="outline" className="border-2 border-white hover:bg-white rounded-full px-8 py-6 text-lg text-secondary-foreground" onClick={() => window.open("https://wa.me/917065389036", "_blank")}>
                  Schedule Site Visit
                </Button>
              </div>
            </div>
